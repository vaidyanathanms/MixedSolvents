...
# Compute RDF
echo "Run rdf-${saltcation}-all"
srun ${gmx} rdf -f traj_npt_main.trr -s npt_main.tpr -o rdf_${salt_cation}_all.xvg -cn nrdf_${salt_cation}_all.xvg -b ${begtime} -e ${endtime} -rmpbc yes -ref -sf ${inpdir}/ref${salt_cation}.txt -sel -sf ${inpdir}/rdfsel_all.txt
wait

echo "Run rdf-${orgcation}-all"
srun ${gmx} rdf -f traj_npt_main.trr -s npt_main.tpr -o rdf_${org_cation}_all.xvg -cn nrdf_${org_cation}_all.xvg -b ${begtime} -e ${endtime} -rmpbc yes -ref -sf ${inpdir}/ref${org_cation}.txt -sel -sf ${inpdir}/rdfsel_all.txt
wait
